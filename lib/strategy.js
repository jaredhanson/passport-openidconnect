  ...
  if (req.query && req.query.code) {
    function loaded(err, ctx, state) {
      ...
      self._oauth2.getOAuthAccessToken(code, params, function(err, accessToken, refreshToken, params) {
        ...
        if (!jwtClaims.iss) { return self.error(new Error('ID token missing issuer claim')); }
        if (!jwtClaims.sub) { return self.error(new Error('ID token missing subject claim')); }
        if (!jwtClaims.aud) { return self.error(new Error('ID token missing audience claim')); }
        if (!jwtClaims.exp) { return self.error(new Error('ID token missing expiration time claim')); }
        if (!jwtClaims.iat) { return self.error(new Error('ID token missing issued at claim')); }
        ...
        if (typeof jwtClaims.aud === 'string') {
          if (jwtClaims.aud !== self._oauth2._clientId) return self.fail({ message: 'ID token not intended for this relying party.' }, 403);
        } else if (Array.isArray(jwtClaims.aud)) {
          if (jwtClaims.aud.indexOf(self._oauth2._clientId) === -1) return self.fail({ message: 'ID token not intended for this relying party.' }, 403);
          if (jwtClaims.aud.length > 1 && !jwtClaims.azp) return self.fail({ message: 'ID token missing authorizied party claim.' }, 403);
        } else {
        ...
        if (jwtClaims.azp && jwtClaims.azp !== self._oauth2._clientId) return self.fail({ message: 'ID token not issued to this relying party.' }, 403);
        ...
        if (jwtClaims.exp <= (Date.now() / 1000)) return self.fail({ message: 'ID token has expired.' }, 403);
        ...
        if (ctx.maxAge && (!jwtClaims.auth_time || ((ctx.issued.valueOf() - (ctx.maxAge * 1000)) > (jwtClaims.auth_time * 1000)))) {
