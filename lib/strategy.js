...
Strategy.prototype.authenticate = function(req, options) {
  ...
  if (req.query && req.query.code) {
    ...
    this._setup(identifier, function(err, config) {
      ...
      //
      // Override prompt param from request, allows you to dynamically change
      // the config to check OIDC endpoints for logged in status
      //
      if (req.body && req.body['prompt']) {
        config.prompt = req.body['prompt'];
      } else if (req.query && req.query['prompt']) {
        config.prompt = req.query['prompt'];
      }
