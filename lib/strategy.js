...
function Strategy(options, verify) {
  ...
  this._skipUserProfile = (options.skipUserProfile === undefined) ? function() { return true; } : options.skipUserProfile;
  ...
  if (req.query && req.query.code) {
    function restored(err, ctx, state) {
      ...
      self._oauth2.getOAuthAccessToken(code, params, function(err, accessToken, refreshToken, params) {
        ...
        self._shouldLoadUserProfile(req, function(err, load) {
          if (err) { return self.error(err); };
          ...
          function loaded(uiProfile, json, body) {
            ...
            var idProfile = UserInfo.parse(claims);
            var profile = {};
            utils.merge(profile, idProfile);
            utils.merge(profile, uiProfile);
            
            // WIP: skip the user info endpoint, unless arity
            //.  update tests to reflect callack arity
            // remove jsonwebtoken devdep
            ...
            try {
              if (self._passReqToCallback) {
                ...
                if (arity == 9) {
                  self._verify(req, iss, sub, profile, claims, accessToken, refreshToken, params, verified);
                } else if (arity == 8) {
                  self._verify(req, iss, profile, idToken, accessToken, refreshToken, params, verified);
                } else if (arity == 7) {
                  self._verify(req, iss, profile, idToken, accessToken, refreshToken, verified);
                } else if (arity == 5) {
                  self._verify(req, iss, profile, idToken, verified);
                } else { // arity == 4
                  self._verify(req, iss, profile, verified);
                }
              } else {
                var arity = self._verify.length;
                if (arity == 8) {
                  self._verify(iss, sub, profile, claims, accessToken, refreshToken, params, verified);
                } else if (arity == 7) {
                  self._verify(iss, profile, idToken, accessToken, refreshToken, params, verified);
                } else if (arity == 6) {
                  self._verify(iss, profile, idToken, accessToken, refreshToken, verified);
                } else if (arity == 4) {
                  self._verify(iss, profile, idToken, verified);
                } else { // arity == 3
                  self._verify(iss, profile, verified);
                }
              }
            } catch (ex) {
              return self.error(ex);
          ...
          self._oauth2.get(userInfoURL, accessToken, function (err, body, res) {
            ...
            var profile = UserInfo.parse(json);
...
Strategy.prototype._shouldLoadUserProfile = function(req, done) {
  if (typeof this._skipUserProfile == 'function' && this._skipUserProfile.length > 1) {
    ...
    var skip = (typeof this._skipUserProfile == 'function') ? this._skipUserProfile(req) : this._skipUserProfile;
    if (!skip) { return done(null, true); }
    return done(null, false);
  }
}
