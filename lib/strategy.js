...
function Strategy(options, verify) {
  ...
  if (options.authorizationURL && options.tokenURL) {
    ...
    console.error('.well-known/openid-configuration');
    this.configure(require('./setup/dynamic')(options));
  }
}
...
Strategy.prototype.authenticate = function(req, options) {
  ...
  if (req.query && req.query.code) {
    ...
    function loaded(err, ok, state) {
      ...
      var parsedTokenUrl = url.parse(meta.tokenURL);

      var options = {
        headers : {
          'Content-Type': 'application/x-www-form-urlencoded',
          'Content-Length': postBody.length,
          'Authorization': 'Basic ' + Buffer.from(meta.clientID + ':' + meta.clientSecret).toString('base64')
        },
      ...
      var postreq = https.request(options, function (res) {
        ...
        res.on('end', function () {
          ...
          var params = parsedResp;
          var idToken = parsedResp.id_token;
          var accessToken = parsedResp.access_token;
          var refreshToken = parsedResp.refresh_token;
          ...
          self._shouldLoadUserProfile(iss, sub, function(err, load) {
            ...
            function onProfileLoaded(profile) {
              ...
              if (self._passReqToCallback) {
                console.error(11111);
                console.error(self._verify.length);
