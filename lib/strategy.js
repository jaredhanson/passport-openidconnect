...
Strategy.prototype.authenticate = function(req, options) {
  ...
  if (req.query && req.query.code) {
    ...
    function loaded(err, ok, state) {
      ...
      var meta = state;
      if (!meta) {
        // TODO: Clean this up, loading config from options to constructor rather than state
        meta = {
          issuer: self._options.issuer,
          //authorizationURL: self._options.authorizationURL,
          //tokenURL: self._options.tokenURL,
          //userInfoURL: self._options.userInfoURL,
          //clientID: self._options.clientID,
          //clientSecret: self._options.clientSecret,
          //callbackURL: self._options.callbackURL,
          //customHeaders: self._options.customHeaders,
          params: {}
        }
        
        var callbackURL = self._options.callbackURL;
        if (callbackURL) {
      ...
      self._oauth2.getOAuthAccessToken(code, { grant_type: 'authorization_code', redirect_uri: callbackURL }, function(err, accessToken, refreshToken, params) {
        ...
        if (jwtClaims.iss !== self._issuer) return self.error(new Error('id token not issued by correct OpenID provider - ' +
                                                                       'expected: ' + self._issuer + ' | from: ' + jwtClaims.iss));

        // https://openid.net/specs/openid-connect-basic-1_0.html#IDTokenValidation - checks 2 and 3.
        if (typeof jwtClaims.aud === 'string') {
          if (jwtClaims.aud !== self._oauth2._clientId) return self.error(new Error('aud parameter does not include this client - is: '
                                                                           + jwtClaims.aud + ' | expected: ' + self._oauth2._clientId));
        } else if (Array.isArray(jwtClaims.aud)) {
          if (jwtClaims.aud.indexOf(self._oauth2._clientId) === -1) return self.error(new Error('aud parameter does not include this client - is: ' +
                                                                                       jwtClaims.aud + ' | expected to include: ' + self._oauth2._clientId));
        ...
        if (jwtClaims.azp && jwtClaims.azp !== self._oauth2._clientId) return self.error(new Error('this client is not the authorized party - ' +
                                                                                          'expected: ' + self._oauth2._clientId + ' | is: ' + jwtClaims.azp));
