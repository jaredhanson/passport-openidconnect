...
function Strategy(options, verify) {
  ...
  if (options.authorizationURL && options.tokenURL) {
    ...
    this.configure(require('./setup/manual')(options)); // eslint-disable-line global-require
    // this.configure(require('./setup/dynamic')(options));
  } else {
    this.configure(require('./setup/dynamic')(options)); // eslint-disable-line global-require
  }
}
...
Strategy.prototype.authenticate = function (req, options) {
  ...
  if (req.query && req.query.code) {
    ...
    this._setup(identifier, function (err, config) {
      ...
      var simpleOptionalParams = ['max_age', 'ui_locals', 'id_token_hint', 'login_hint', 'acr_values'];
      simpleOptionalParams
        .filter(function (x) { return x in config; })
        .forEach(function (y) { params[y] = config[y]; });
