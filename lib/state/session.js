...
SessionStore.prototype.store = function(req, ctx, state, meta, callback) {
  if (!req.session) { return callback(new Error('OpenID Connect requires session support. Did you forget to use `express-session` middleware?')); }
 ...
 * @api protected
 */
SessionStore.prototype.verify = function(req, handle, callback) {
  if (!req.session) { return callback(new Error('OpenID Connect requires session support. Did you forget to use `express-session` middleware?')); }
